...
{
    public class SpringSelectionService : SpringSelection.SpringSelectionBase
    {
        public override Task<SpringResponse> SpringSignSelection(SpringRequest request, ServerCallContext context)
        {
            string startDate;
            string endDate;
            string sign;
            string pattern = "MM/dd/yyyy";
            var file = File.ReadAllLines("Spring.txt");
            ...
            for (var i = 0; i < file.Length; i += 3)
            {
                    ...
                    && DateTime.TryParseExact(request.Date, pattern, null, DateTimeStyles.None, out requestParsed))
                {
                    ...
                    {
                        return Task.FromResult(new SpringResponse { Sign = sign });
                    }

                }

            }
            return Task.FromResult(new SpringResponse { Sign = "Could not find a sign for this date!" });
