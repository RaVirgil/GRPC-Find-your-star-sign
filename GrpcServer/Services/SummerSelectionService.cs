...
{
    public class SummerSelectionService : SummerSelection.SummerSelectionBase
    {
        public override Task<SummerResponse> SummerSignSelection(SummerRequest request, ServerCallContext context)
        {
            string startDate;
            string endDate;
            string sign;
            string pattern = "MM/dd/yyyy";
            var file = File.ReadAllLines("Summer.txt");
            ...
            for (var i = 0; i < file.Length; i += 3)
            {
                   ...
                   && DateTime.TryParseExact(request.Date, pattern, null, DateTimeStyles.None, out requestParsed))
                {
                    ...
                    {
                        return Task.FromResult(new SummerResponse { Sign = sign });
                    }

                }

            }
            return Task.FromResult(new SummerResponse { Sign = "Could not find a sign for this date!" });
